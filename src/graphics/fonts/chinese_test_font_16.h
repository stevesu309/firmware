...
#define CHINESE_FONT_CMAP_FORMAT0_FULL 1
#define CHINESE_FONT_CMAP_SPARSE_TINY 2
  ...
  } chinese_font_kern_pair_t;

  // 字体描述结构
  typedef struct
  {
    const uint8_t *glyph_bitmap;               // 字形位图数据
    const chinese_font_glyph_dsc_t *glyph_dsc; // 字形描述
    const chinese_font_cmap_t *cmaps;          // Unicode映射表
    const chinese_font_kern_pair_t *kern_dsc;  // 字距调整描述
  ...
  typedef struct
  {
    const chinese_font_dsc_t *dsc; // 字体描述
  ...
  extern const uint8_t chinese_font_glyph_bitmap[];
  extern const chinese_font_glyph_dsc_t chinese_font_glyph_dsc[];
  extern const chinese_font_cmap_t chinese_font_cmaps[];
  extern const uint16_t chinese_font_cmap_num;
  extern const chinese_font_t chinese_font_16;

  // 查找函数
  extern const chinese_font_glyph_dsc_t *chinese_font_get_glyph(uint32_t unicode);
