...
#define CHINESE_FONT_CMAP_SPARSE_TINY 1
  ...
  } chinese_font_kern_pair_t_8bit;

  // 字体描述结构
  typedef struct
  {
    const uint8_t *glyph_bitmap;                    // 字形位图数据
    const chinese_font_glyph_dsc_t_8bit *glyph_dsc; // 字形描述
    const chinese_font_cmap_t_8bit *cmaps;          // Unicode映射表
    const chinese_font_kern_pair_t_8bit *kern_dsc;  // 字距调整描述
    uint8_t kern_scale;                             // 字距调整缩放
    uint16_t cmap_num;                              // 映射表数量
    uint8_t bpp;                                    // 每像素位数
    uint8_t kern_classes;                           // 字距调整类
    uint8_t bitmap_format;                          // 位图格式
  } chinese_font_dsc_t_8bit;

  // 字体结构
  typedef struct
  {
    const chinese_font_dsc_t_8bit *dsc; // 字体描述
  ...
  extern const uint8_t chinese_font_glyph_bitmap_8bit[];
  extern const chinese_font_glyph_dsc_t_8bit chinese_font_glyph_dsc_8bit[];
  extern const chinese_font_cmap_t_8bit chinese_font_cmaps_8bit[];
  extern const uint16_t chinese_font_cmap_num_8bit;
  extern const chinese_font_t_8bit chinese_font_16_8bit;

  // 查找函数
  extern const chinese_font_glyph_dsc_t_8bit *chinese_font_get_glyph_8bit(uint32_t unicode);
