...
namespace graphics
{
    ...
    void drawCommonHeader(OLEDDisplay *display, int16_t x, int16_t y, const char *titleStr, bool force_no_invert, bool show_date)
    {
        ...
        // LOG_INFO("Battery: %d%%, Charging: %d, USB Powered: %d", chargePercent, isCharging, usbPowered);
        ...
        { // This is a basic check to determine USB Powered is flagged but not charging
            batteryX += 1;
            batteryY += 2;
            if (currentResolution == ScreenResolution::High)
            {
